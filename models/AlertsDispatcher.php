 ...
 */
class AlertsDispatcher extends \yii\base\Component
{
	public $mode;
	...
	public function sendAlerts($compose, $ownerId)
	{
		...
		switch(is_array($this->_alerts ) && !empty($this->_alerts ))
		{
			...
			foreach($to as $scope=>$types)
			{
				...
				{
					case self::SINGLE;
					$this->sendAsSingle($scope, $types);
					break;
					
					default:
					$this->sendAsBatch($scope, $types);
					break;
				}
			}
			break;
	 ...
	 * @return boolean
	 */
	protected function sendAsBatch($scope, $types)
	{
		$ret_val = false;
	 ...
	 * @return boolean
	 */
	protected function sendAsSingle($scope, $types)
	{
		$ret_val = false;
		switch(is_array($types))
		{
			...
			foreach($types as $type=>$unMappedAddresses)
			{
				...
				while($address = array_pop($addresses))
				{
					...
					switch($type)
					{
						case 'email':
						$view = ['html' => '@nitm/views/alerts/message/email'];
						$params['content'] = nl2br($params['content'].$this->getFooter($scope, $this->_alerts[current($unMappedAddresses)['user']->getId()]));
						break;
	...
	{
		return $this->_message->setFrom(\Yii::$app->params['components.alerts']['sender'])
			->send();
	}
	...
	protected function filterMethods($value)
	{
		$ret_val = [];
	...
	{	
		$alert = is_array($alert) ? $alert : $this->criteria;
		switch($scope)
		{
			case 'global':
		...
		$footer .= "and Action <b>".$this->properName($this->reportedAction)."</b>";
