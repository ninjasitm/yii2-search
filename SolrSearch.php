...
use nitm\helpers\StringHelper;
use nitm\helpers\ArrayHelper;

class SolrSearch extends BaseSearch
{
	public $params = [];
	public $condition = [];
	...
	public function addParam($param, $values)
	{
		...
		foreach($param as $i=>$p)
		{
			switch(isset($values[$i]))
			{
				...
				{
					case 'sort':
					$v = ArrayHelper::splitc(array_keys($values[$i]), array_values($values[$i]), ' ', ', ', false, false, false);
	...
	protected function logterm()
	{
		if(!empty($this->term))
		{
			if(sizeof($this->resids) >= 1)
			{
				...
				{
					parent::insert(['term','searches','resource','last_searched', 'resids'], [$this->term, '1', $this->resource, strtotime('now'), json_encode($this->resids)], $this->wordTable, null, true, null, true);
				}
				else
				{
					parent::update(['searches','resource','last_searched', 'resids'], ["`searches`+1", $this->resource, strtotime('now'), json_encode($this->resids)], ['key' => 'wid', 'data' => $key['wid'][0]], $this->wordTable, null, 1, true);
				}
			}
		}
	}
